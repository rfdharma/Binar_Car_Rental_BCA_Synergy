...
const Cards = () => {
  ...
  const getAllCars = async () => {
    const response = await axios.get(CARS_ENDPOINT_URL);
  ...
  const onEventSubmitFilter = (event) => {
    ...
    if (Object.keys(filterCarParams).length === 0) {
      getAllCars(); // If no filters, get all cars
    } else {
  ...
  const onShowFilterCars = async (filter) => {
    if (filter.capacity != null) {
      ...
      console.log("Filtered cars:", filteredCars.data);
